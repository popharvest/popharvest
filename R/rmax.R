...
#'
#' @details
#' When the maximum annual recruitment rate, Rmax, is unknown from the user,
#' the value can be calculated using the Demographic Invariants Method (DIM) (Niel & Lebreton, 2005). This approach calculates \eqn{Rmax = lambdaMax – 1} (Dillingham & Fletcher, 2008) on the basis of 2 parameters:
#' adult survival rate (\code{surv} arguments) and age at first breeding \code{alpha} arguments). When adult survival and age at first breeding are known
...
#' adult survival rate can be estimated from body mass following the relationship described in Johnson et al. (2012) : \eqn{p^(1/(exp(3.22+0.24*log(body mass)+e)-alpha))} (see \code{surv} function).
...
#' @param surv.beta surv.beta boolean, default FALSE. If \code{TRUE}, simulates adult annual survival rate from a beta distribution with mu = \code{mean.surv} and standard deviation = \code{sd.surv} (see below).
#' @param mean.surv mean.surv number ; arithmetic mean of adult annual survival rate. Only used if \code{surv.beta = TRUE}.
...
rmax <- function (Nsim = 1,
  ...
  if(isTRUE(alpha.lognorm)) { # If alpha is extracted from  a normal distribution then :
    ...
    alpha.calc <- sample(seq(alpha.param[1]-alpha.param[2], alpha.param[1]+alpha.param[2], 0.001), Nsim) # 'Nsim' values are extracted from a log-normal distribution with with the mean 'mean.alpha' and the standard deviation 'sd.alpha'
  }
  ...
  if(!is.na(mass.fixed)){ # If the value of mass.fixed is different to 'NA' (i.e. the value of the body mass is known) then :
    survival.tab <- surv(Nsim, mass.fixed = mass.fixed, alpha = alpha.calc, type.p = type.p, type.e = type.e) # The values of survival are calculated from the surv function and save in a data frame named 'survival.tab'
  ...
  if(isTRUE(mass.lognorm)){ # If the body mass is extracted from a normal distribution then :
    survival.tab <- surv(Nsim, mass.lognorm = TRUE, mean.mass = mean.mass, sd.mass = sd.mass, alpha = alpha.calc) # The values of survival are calculated from the surv function and save in a data frame named 'survival.tab'
    survival.calc <- survival.tab$survival # The values of survival are extract from the data frame survival.tab and save in the object 'survival.calc'
  }
  if(isTRUE(surv.beta)){ # If survival is extracted from a normal distribution then :
    beta.param <- MOM.beta(mean.surv, sd.surv)
    survival.calc <- sample(seq(beta.param[1]-beta.param[2], beta.param[1]+beta.param[2], 0.001) , Nsim) # 'Nsim' values are extracted from a beta distribution with the alpha parameter 'a' and the beta parameter 'b'
  ...
  var.storage <- c("min.alpha", "max.alpha","mean.alpha", "sd.alpha", "mass.fixed",
                   "mean.mass","sd.mass") # Save in an object all sumpplementary possible columns
