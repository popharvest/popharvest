...
#'
#' @details
#' Calculates the Prescribed Take Level (PTL) integrating a density-dependance parameter and Sustainable Harvest Index (SHI).
#'
...
#' \eqn{Fobj.(Rmax.theta)/(1+theta)} for a breeding population (Johnson et al., 2012).
#'
#'
#' Fobj (\code{Fobj} argument) is a factor that reflects management objectives of the decision-maker. It is related to
#' the desired long-term population size relative to its carrying capacity (Runge et al. 2009). For example,
#' a factor of 0.25 i.e. 25% of the maximum sustainable yield is taken (Johnson et al., 2012).
#'
#'
...
#' with \code{mean.Rmax} and \code{sd.Rmax}, \code{lambdaMax.lognorm = TRUE} with \code{mean.lambdaMax} and \code{sd.lambdaMax}). When Rmax (or lambdaMax) is unknown from the user,
...
#' adult survival rate and age at first breeding. When adult survival rate (\code{surv} arguments) and age at first reproduction (\code{alpha} arguments) are known
...
#' survival rate can be estimated from body mass following the relationship described in Johnson et al. (2012) : \eqn{p^(1/(exp(3.22+0.24*log(body mass)+e)-alpa))} (see \code{surv} function).
#' In this case, body mass can be provided as either a point estimates (\code{mass.fixed})
...
#' and high first-generation age. This argument is used to select the right formula from Niel & Lebreton (2005) for the calculation of lambdaMax (see \code{lambdaMax.L} for long-lived species and \code{lambdaMax.S} for short-lived species)
#' and, therefater Rmax.
#'
#'
...
#'
#'
#' The Sustainable Harvest Index (SHI) is the harvest level expressed as a proportion relative to PTL: \eqn{harvest level/PTL}. The harvest level can
...
#' When harvest exceeds PTL, this suggests an overexploitation regime according to a fixed management objective.
#'
#'
#' The function PTL provides a great flexibility of use according to the known arguments and analysis that the user wants realize.
...
#' @param lambdaMax.lognorm boolean, default \code{FALSE}. If \code{TRUE}, simulates maximum mulitplicative growth rate from a normal distribution with mu = \code{mean.lambdaMax} and standard deviation = \code{sd.lambdaMax}.
#' @param mean.lambdaMax a number ; arithmetic mean of maximum multiplicative growth rate. To be used only used if \code{lambdaMax.lognorm = TRUE}.
#' @param sd.lambdaMax a number ; standard deviation of maximum multiplicative growth rate. To be used only if \code{lambdaMax.lognorm = TRUE}.
#' @param living.rate character string ; living rate : either \code{"long"} or \code{"short"}, see 'Details'. No ohter arguments allowed.
#' @param surv.fixed number ; fixed estimate of adult annual survival rate provided by the user. To be used instead of mean.surv when \code{surv.beta = FALSE}.
#' @param surv.beta boolean, default \code{FALSE}. If \code{TRUE}, simulates adult annual survival rate from a bate distribution with mu = \code{mean.surv} and standard deviation = \code{sd.surv} (see below).
#' @param mean.surv number ; arithmetic mean of adult annual survival rate. To be used only if \code{surv.beta = TRUE}.
#' @param sd.surv number ; standard deviation of adult annual survival rate. To be used only if \code{surv.beta = TRUE}.
...
PTL <- function(full.option = FALSE,
  ...
  for (a in 1: NSp) { # Loop for calculations from 1 to the number of species studied
    ...
    for (b in 1: length(Fobj)){ # Loop for calculations according to the objective factor
      ...
      # Step 10 : Save all the datas from the 'OutputT2' in the data frame named 'Output2' ----
