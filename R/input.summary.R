...
#'   \item \code{mean.surv} arithmetic mean of annual survival rate for the adult age class
#'   \item \code{med.surv} median of annual survival rate for the adult age class
#'   \item \code{se.surv} standard error of annual survival rate for the adult age class
...
#'   \item \code{se.alpha} standard error of annual survival rate for the adult age class
...
#'   \item \code{mean.Rmax} arithmetic mean of age at first breeding
...
input.summary <- function(data){
  ...
  ## Step 1 : Creation of the end storage vector to save results of the calculations----
  Output <- NULL
  ...
  for (j in levels(data$code)){
    ...
    col <- which(colnames(newdata) == "survival") # Finding if there is a column named "survival" in the data frame named 'newdata'
    if(length(col)> 0){ # If there is a column named "survival" then :
      mean.surv <- round(mean(newdata$survival), 3) # Calculating the mean rounded to the thousandth
      med.surv <- round(median(newdata$survival), 3) # Calculating the median rounded to the thousandth
      if(mean.surv == round(newdata$survival[1], 3)){ # If the mean is equal to the first row of the column named 'survival' (means that all values in the 'survival' column are the same) then :
        se.surv <- 0 # Atributting 0 to the standard error object
        surv.CI95.low <- 0 # Atributting 0 to the lower bound of the interval of confidence at 95 \%
        surv.CI95.up <- 0 # Atributting 0 to the upper bound of the interval of confidence at 95 \%
        ...
        se.surv <- round(se(newdata$survival),3) # Calculating the standard error rounded to the thousandth
      CI.list <- t.test(newdata$survival) # Calculating the interval of confidence at 95 \%
      ...
      surv.CI95.low <- round(CI[1], 3) # Extracting the lower bound of the interval of confidence at 95 \% rounded to the thousandth
      surv.CI95.up <- round(CI[2], 3) # Exracting the upper bound of the interval of confidence at 95 \% rounded to the thousandth
      }
    }else{ # If there is no column named "survival" then attributing NA to the descriptive statistics
      mean.surv <- NA
      med.surv <- NA
      se.surv <- NA
      surv.CI95.low <- NA
    ...
    col <- which(colnames(newdata) == "alpha") # Finding if there is a column named "alpha" in the data frame named 'newdata'
    if(length(col)> 0){ # If there is a column named "alpha" then :
      mean.alpha <- round(mean(newdata$alpha), 3) # Calculating of the mean rounded to the thousandth
      med.alpha <- round(median(newdata$alpha), 3) # Calculating of the median rounded to the thousandth
      if(mean.alpha == round(newdata$alpha[1], 3)){ # If the mean is equal to the first row of the column named 'alpha' (means that all values in the 'alpha' column are the same) then :
        se.alpha <- 0 # Attributing 0 to the standard error
        alpha.CI95.low <- 0 # Attributing 0 to the lower bound of interval of confidence at 95 \%
        alpha.CI95.up <- 0 # Attributing 0 to the upper bound of interval of confidence at 95 \%
        # At this step we must assign 0 manually because otherwise an error message is returned
      }else{ # If the mean is different to the first row of the column named 'alpha' then :
        se.alpha <- round(se(newdata$alpha),3) # Calculating of the standard error rounded to the thousandth
        CI.list <- t.test(newdata$alpha) # Calculating the interval of confidence at 95 \%
        CI <- CI.list[[4]] # Extracting the fourth row from the list which returns the lower and the upper bound of the interval of confidence at 95 \%
        alpha.CI95.low <- round(CI[1], 3) # Extracting the lower bound of the interval of confidence at 95 \% rounded to the thousandth
        alpha.CI95.up <- round(CI[2], 3) # Extracting the upper bound of the interval of confidence at 95 \% rounded to the thousandth
      }
    }else{ # If there is no column named "alpha" then attributing NA to the descriptive statistics
      mean.alpha <- NA
      med.alpha <- NA
      se.alpha <- NA
      alpha.CI95.low <- NA
    ...
    col <- which(colnames(newdata) == "lambdaMax") # Finding if there is a column named "lambdaMax" in the data frame named 'newdata'
    if(length(col)> 0){ # If there is a column named "lambdaMax" then :
      mean.lambdaMax <- round(mean(newdata$lambdaMax), 3) # Calculating of the mean rounded to the thousandth
      med.lambdaMax <- round(median(newdata$lambdaMax), 3) # Calculating of the median rounded to the thousandth
      if(mean.lambdaMax == round(newdata$lambdaMax[1], 3)){ # If the mean is equal to the first row of the column named 'lambdaMax' (means that all values in the 'lambdaMax' column are the same) then :
        se.lambdaMax <- 0 # Attributing 0 to the standard error
        lambdaMax.CI95.low <- 0 # Attributing 0 to the to the lower bound of interval of confidence at 95 \%
        lambdaMax.CI95.up <- 0 # Attributing 0 to the to the lower bound of interval of confidence at 95 \%
        # At this step we must assign 0 manually because otherwise an error message is returned
      }else{ # If the mean is different to the first row of the column named 'lambdaMax' then :
        se.lambdaMax <- round(se(newdata$lambdaMax),3) # Calculating of the standard error rounded to the thousandth
        CI.list <- t.test(newdata$lambdaMax) # Calculating the interval of confidence at 95 \%
        CI <- CI.list[[4]] # Extracting the fourth row from the list which returns the lower and the upper bound of the interval of confidence at 95 \%
        lambdaMax.CI95.low <- round(CI[1], 3) # Extracting the lower bound of the interval of confidence at 95 \% rounded to the thousandth
        lambdaMax.CI95.up <- round(CI[2], 3) # Extracting the upper bound of the interval of confidence at 95 \% rounded to the thousandth
      }
    }else{ # If there is no column named "alpha" then attributing NA to the descriptive statistics
      mean.lambdaMax <- NA
      med.lambdaMax <- NA
    ...
    mean.Rmax <- round(mean(newdata$Rmax), 3) # Calculating of the mean rounded to the thousandth
    med.Rmax <- round(median(newdata$Rmax), 3) # Calculating of the median rounded to the thousandth
    ...
    se.Rmax <- 0 # Attributing 0 to the standard error
      Rmax.CI95.low <- 0 # Attributing 0 to the to the lower bound of interval of confidence at 95 \%
      Rmax.CI95.up <- 0 # Attributing 0 to the to the upper bound of interval of confidence at 95 \%
    }else{ # If the mean is different to the first row of the column named 'Rmax' then :
      se.Rmax <- round(se(newdata$Rmax),3) # Calculating of the standard error rounded to the thousandth
    ...
    # In this case, it implied there is a column named "Fs" in the data frame 'newdata' and the PEG function was used previously
    if (length(col) > 0){
      Fs <- unique(newdata$Fs) # Deleting duplicates in the column 'Fs' in the data frame named 'data'
      OutputT <- data.frame( # Creating a data frame named 'OutputT' containing descriptive statistics of Rmax
          code = newdata$code[1],
          Fs = Fs,
          med.Rmax = med.Rmax,
          mean.Rmax = mean.Rmax,
    ...
    # In this case, there is a column named 'Fobj' exists
      ...
      OutputT <- data.frame( # Creating a data frame named 'OutputT' with descriptive statistics of Rmax
          code = newdata$code[1],
          Fobj = Fobj,
          med.Rmax = med.Rmax,
          mean.Rmax = mean.Rmax,
    ...
    var.storage <- c("med.surv", "mean.surv", "se.surv", "surv.CI95.low", "surv.CI95.up",
                     ...
                     "med.lambdaMax", "mean.lambdaMax", "se.lambdaMax", "lambdaMax.CI95.low", "lambdaMax.CI95.up") # Saving in an object all sumpplementary possible columns
    ...
    OutputT[,var.storage[not.na]]<- mget(var.storage)[not.na] # Insert values columns whose values are different from NA in the temporary data frame named 'OutputT'



    ## Step 6 : Final ouptut ----
    Output <- rbind(Output, OutputT) # Combining by rows all temporary data frame for each level of j

  } # End of loop j



  ## Step 7 : Dataframe transposition ----
  ...
  Output <- as.data.frame(Output) # Converting the matrix nmaed 'Output' as a data frame
  ...
  ## Step 9 : Deleting the first row of the table which does not correspond to calculations ----
