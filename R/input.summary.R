...
input.summary <- function(data){
  ...
  for (j in levels(data$code)){
    ...
    if(length(col)> 0){ # If TRUE then :
      ...
      percent2.5.surv <- quantile(newdata$survival, probs=0.025)
      percent97.5.surv <- quantile(newdata$survival, probs=0.975)
    ...
    if(length(col)> 0){ # If TRUE, then :
      ...
      percent2.5.alpha <- quantile(newdata$alpha, probs=0.025)
      percent97.5.alpha <- quantile(newdata$alpha, probs=0.975)
    ...
    if(length(col)> 0){ # If TRUE, then :
      ...
      percent2.5.lambdaMax <- quantile(newdata$lambdaMax, probs=0.025)
      percent97.5.lambdaMax <- quantile(newdata$lambdaMax, probs=0.975)
    }else{ # If there is no column named "alpha" then assign NA to the descriptive statistics
      mean.lambdaMax <- NA
      med.lambdaMax <- NA
    ...
    percent2.5.Rmax <- quantile(newdata$Rmax, probs=0.025)
    percent97.5.Rmax <- quantile(newdata$Rmax, probs=0.975)
