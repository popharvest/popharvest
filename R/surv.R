...
surv <- function (Nsim = 1,
  ...
  ## Step 1 : Creating storage vectors to save the results of the calculations and loading functions ----
  Output <- NULL
  e.storage <- NULL
  p.storage <- NULL
  ...
  for (i in 1:NSp){ # Loop to estimate adult annual survival rate i times
    ...
    if(isTRUE(mass.lognorm)){ # If mass.lognorm argument is TRUE
      ...
      massT <- sample(seq(mass.param[1]-mass.param[2], mass.param[1]+mass.param[2], 0.001), Nsim) # 'Nsim' values are extracted from a log-normal distribution with with the mean 'mean.mass' and the standard deviation 'sd.mass'
    ...
    ## Step 3 : Final output ----
    OutputT <- data.frame(Sim = Nsim.storage, mass = massT, alpha = alphaT, # Saving temporarly values used to estimate adult annual survival rate in a dataframe name OutputT
