...
#'
#' The function returns descriptive statistics (mean, median, se and 95 \% confidence intervals) for the output parameters used by PEG and PTL functions. These parameters are : potential excess growth (PEG) or prescribed take level (PTL), sustainable harvest index (SHI), the maximum annual recruitment rate (Rmax) and the number of iterations for which the observed harvest exceeds the sustainable harvest index (unsustain.harvest). Descriptive statistics are drawn from the n simulations called within the \code{PEG} and \code{PTL} functions. When one or more parameters are fixed by the user, the corresponding value is reported for the mean and the median, without any further measure of dispersion (i.e. se and 95 \% CI are equal to zero). Statistics are reported in columns.
#'
