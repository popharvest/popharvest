  ...
  for (j in levels(data$code)){
    ...
    if(length(col)> 0){ # If TRUE, the PEG function was used previously
      ...
      if(mean.PEG == round(newdata$PEG[1])){ # If the mean is equal to the first row of the column named 'PEG' (means that all values in the 'PEG' column are the same) then :
        se.PEG <- 0 # Assign 0 to the standard error object
        PEG.CI95.low <- 0 # Assign 0 to the lower bound of the confidence interval at 95 \%
        PEG.CI95.up <- 0 # Assign 0 to the upper bound of the confidence interval at 95 \%
        ...
        se.PEG <- round(se(newdata$PEG)) # Standard error rounded to the whole
        CI.list <- t.test(newdata$PEG) # Confidence interval at 95 \%
        ...
        PEG.CI95.low <- round(CI[1])  # Extract the lower bound of the confidence interval at 95 \% rounded to the whole
        PEG.CI95.up <- round(CI[2])  # Extract the upper bound of the confidence interval at 95 \% rounded to the whole
      } # End of condition
    } # End of condition
    ...
    if(length(col)> 0){ # If TRUE, the PEG function was used previously
      ...
      if (mean.PTL == round(newdata$PTL[1])) { # If the mean is equal to the first row of the column named 'PTL' (means that all values in the 'PTL' column are the same) then :
        se.PTL <- 0 # Assign 0 to the standard error
        PTL.CI95.low <- 0 # Assign 0 to the lower bound of the confidence interval at 95 \%
        PTL.CI95.up <- 0 # Assign 0 to the upper bound of the confidence interval at 95 \%
        # At this step we must assign 0 manually because otherwise an error message is returned
      } else{  # If the mean is different to the first row of the column named 'PTL' then :
        se.PTL <- round(se(newdata$PTL)) # Standard error rounded to the whole
        CI.list <- t.test(newdata$PTL)  # Confidence interval at 95 \%
        CI <- CI.list[[4]] # Extract the fourth row from the list which returns the lower and the upper bound of the confidence interval at 95 \%
        PTL.CI95.low <- round(CI[1])  # Extract the lower bound of the confidence interval at 95 \% rounded to the whole
        PTL.CI95.up <- round(CI[2])  # Extract the upper bound of the confidence interval at 95 \% rounded to the whole
      } # End of condition
    } # End of condition
    ...
    if(mean.SHI == round(newdata$SHI[1], 3)){ # If the mean is equal to the first row of the column named 'SHI' (means that all values in the 'SHI' column are the same) then :
      se.SHI <- 0 # Assign 0 to the standard error
      SHI.CI95.low <- 0 # Assign 0 to the lower bound of the confidence interval at 95 \%
      SHI.CI95.up <- 0 # Assign 0 to the upper bound of the confidence interval at 95 \%
      # At this step we must assign 0 manually because otherwise an error message is returned
    }else{ # If the mean is different to the first row of the column named 'SHI' then :
      se.SHI <- round(se(newdata$SHI),3) # Standard error rounded to the thousandth
      try(CI.list <- t.test(newdata$SHI), silent = TRUE) # Confidence interval at 95 \%
      CI <- CI.list[[4]] # Extract the fourth row from the list which returns the lower and the upper bound of the confidence interval at 95 \%
      SHI.CI95.low <- round(CI[1], 3) # Extract the lower bound of the confidence interval at 95 \% rounded to the thousandth
      SHI.CI95.up <- round(CI[2], 3) # Extract the upper bound of the confidence interval at 95 \% rounded to the thousandth
    } # End of condition
    ...
    if (mean.Rmax == round(newdata$Rmax[1], 3)) { # If the mean is equal to the first row of the column named 'Rmax' (means that all values in the 'Rmax' column are the same) then :
      ...
      Rmax.CI95.low <- 0 # Assign 0 to the lower bound of the confidence interval at 95 \%
      Rmax.CI95.up <- 0 # Assign 0 to the upper bound of the confidence interval at 95 \%
      # At this step we must assign 0 manually because otherwise an error message is returned
