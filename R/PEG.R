...
#'
#' @details
#' The function calculates the Potential Excess Growth (PEG) and Sustainable Harvest Index (SHI), in absence of others mortality source.
#'
#'
#' PEG the maximum individuals number that can be removed annually from a population
#' given its maximum annual growth rate (Rmax). PEG is calculated as the product of the safety factor (Fs), Rmax, and the population size (popsize) : \eqn{Fs.Rmax.popsize} (Niel & Lebreton 2005).
#'
#'
...
#' growth rate, lambdaMax, under optimal growth conditions. Rmax and lambdaMax can be provided by either a point estimates (\code{Rmax.fixed}, \code{lambdaMax.fixed}) or a log-normal distribution (\code{Rmax.lognorm = TRUE
...
#' the value is calculated using the Demographic Invariants Method (DIM) (Niel & Lebreton, 2005) (see \code{rmax} function). This approach calculates \eqn{Rmax = lambdaMax – 1} (Dillingham & Fletcher, 2008) on the basis of 2 parameters:
#' adult survival rate (\code{surv} arguments) and age at first breeding (\code{alpha} arguments). When adult survival and age at first reproduction are known
...
#' survival rate can be estimated from body mass following the relationship described in Johnson et al. (2012) : \eqn{p^(1/(exp(3.22+0.24*log(body mass)+e)-alpha))} (see \code{surv} function).
...
#' unlike short-lived species, they rater invest in adult survival and usually have low brood size, and high first breeding age.
#' This argument is used to select the right formula from Niel & Lebreton (2005) for the calculation of lambdaMax (see \code{lambdaMax.L} for long-lived species and \code{lambdaMax.S} for short-lived species) and, therefater Rmax.
#'
#'
...
#'
#'
#' The Sustainable Harvest Index (SHI) is the harvest level expressed as a proportion relative to PEG: \eqn{harvest level/PEG}. The harvest level can
...
#' unlikely that a population is at maximum annual recruitment rate (Rmax). Consequently, when harvest exceeds PEG, this suggests an
