...
#' with \code{min.harvest} and \code{max.harvest} or \code{harvest.norm = TRUE} with \code{mean.harvest} and \code{sd.harvest}). In natura, it is
...
#' @param lambdaMax.norm boolean, default \code{FALSE}. If \code{TRUE}, simulates maximum multiplicative growth rate from a normal distribution with mu = \code{mean.lambdaMax} and standard deviation = \code{sd.lambdaMax}.
#' @param mean.lambdaMax number ; arithmetic mean of maximum multiplicative growth rate. To by used only if \code{lamndaMax.norm = TRUE}.
#' @param sd.lambdaMax number ; standard deviation of maximum multiplicative growth rate. To be used only if \code{lambdaMax.norm = TRUE}.
...
#' @param surv.norm boolean, default FALSE. If \code{TRUE}, simulates adult annual survival rate from a normal distribution with mu = \code{mean.surv} and standard deviation = \code{sd.surv} (see below).
#' @param mean.surv number ; arithmetic mean of adult annual survival rate. Only used if \code{surv.norm = TRUE}.
#' @param sd.surv number ; standard deviation of adult annual survival rate. Only used if \code{surv.norm = TRUE}.
#' @param alpha.fixed number ; fixed estimate of age at first breeding (in years). To be used when \code{alpha.unif = FALSE} and \code{alpha.norm = FALSE}.
...
#' @param sd.alpha number ; age at first breeding standard deviation. Only used if \code{alpha.norm = TRUE}.
