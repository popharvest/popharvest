...
#'
#'
#' PEG the maximum individuals number that can be removed annually from a population given its maximum annual growth rate (Rmax). PEG
...
#' with estimates of population size and maximum annual population growth rate as well as the shape and magnitude of the dependence density
#' phenomena involved. This is a precaution that takes only a fraction of potential growth in excess.
#'
#'
#' Rmax is the maximum annual recruitment rate estimation. This is equivalent to the maximum annual population growth rate, lambdaMax,
#' under optimal growth conditions. Rmax and lambdaMax can be provided by either a point estimates (\code{Rmax.fixed},
#' \code{lambdaMax.fixed}) or a log-normal distribution (\code{Rmax.lognorm = TRUE} with \code{mean.Rmax} and \code{sd.Rmax},
#' \code{lambdaMax = TRUE} with \code{mean.lambdaMax} and \code{sd.lambdaMax}). When Rmax (or lambdaMax) is unknown from the user,
#' the value is calculated using the Demographic Invariants Method (DIM) (Niel & Lebreton, 2005) (see \code{rmax} function). This
#' approach calculates \eqn{Rmax = lambdaMax – 1} (Dillingham & Fletcher, 2008) on the basis of 2 parameters: adult survival rate
#' (\code{surv} arguments) and age at first breeding (\code{alpha} arguments). When adult survival and age at first reproduction are
#' known from the user, they can be provided by either point estimates (\code{surv.fixed}, \code{alpha.fixed} arguments) or distributions
#' (\code{surv.beta = TRUE} with \code{mean.surv} and \code{sd.surv}, \code{alpha.unif = TRUE} with \code{min.alpha} and \code{max.alpha},
#' \code{alpha.lognorm = TRUE} with \code{mean.alpha} and \code{sd.alpha} arguments). Also, survival rate can be estimated from body mass
#' following the relationship described in Johnson et al. (2012) : \eqn{p^(1/(exp(3.22+0.24*log(body mass)+e)-alpha))} (see \code{surv}
#' function). In this case, body mass can be provided as either a point estimate (\code{mass.fixed}) or a log-normal distribution
#' (\code{mass.lognorm = TRUE}, \code{mean.mass} and \code{sd.mass} arguments). The parameter p can be provided by either from a beta
#' distribution B(3.34, 101.24) (\code{type.p = "random"}) or a point estimate equal to 0.032 (i.e. the mean of the beta distribution is
#' given by a/(a + b) with a = 3.34 and b = 101.24, Boitani & Fuller 2000) (\code{type.p = "determinist"}). The parameter e can also be
#' either null (\code{type.e = "determinist"}) or residuals from a normal distribution N(0, 0.087) (\code{type.e = "random"}).
#'
