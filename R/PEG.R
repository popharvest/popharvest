...
#'
#' \code{PEG} is a function used to assess whether a known level of harvesting exceeds the biological potential of renewal of a population, see 'Details'.
#'
#' @details
#' The function calculates the Potential Excess Growth (PEG) and Sustainability Harvest Index (SHI), in absence of others mortality source.
#'
#'
#' PEG is expressed as a number of individuals and corresponds to the maximum number that can be removed annually from a population
#' given its maximum annual growth rate (Rmax). PEG is calculated as the product of the safety factor (Fs), Rmax, and the population size (popsize).
#'
#'
#' Fs (\code{Fs} argument) is a positive number less than 1 that takes into account several aspects such as the uncertainties
#' associated with estimates of population size and maximum annual population growth rate as well as the shape and magnitude of the
#' dependence density phenomena involved. This is a precaution that takes only a fraction of potential growth in
#' excess.
#'
#'
#' Rmax is the maximum annual recruitment rate estimation. This is equivalent to the maximum annual population
#' growth rate, lambdaMax, under optimal growth conditions. Rmax and lambdaMax can be provided by either a point estimates (\code{Rmax.fixed}, \code{lambdaMax.fixed}) or a normal distribution (\code{Rmax.lognorm = TRUE}
...
#' from the user, they can be provided by either point estimates (\code{surv.fixed}, \code{alpha.fixed} arguments) or distributions (\code{surv.beta = TRUE} with \code{mean.surv} and \code{sd.surv},
#' \code{alpha.unif = TRUE} with \code{min.alpha} and \code{max.alpha}, \code{alpha.lognorm = TRUE} with \code{mean.alpha} and \code{sd.alpha} arguments). Also,
...
#' or a normal distribution (\code{mass.lognorm = TRUE}, \code{mean.mass} and \code{sd.mass} arguments). The parameter p can be provided by either from a beta distribution
...
#'
#'
#' The Sustainability Harvest Index or SHI is the harvest level expressed as a proportion relative to PEG: \eqn{harvest level/PEG}. The harvest level can
#' be provided by a point estimates (\code{harvest.fixed}) or distributions (\code{harvest.unif = TRUE}
#' with \code{min.harvest} and \code{max.harvest} or \code{harvest.lognorm = TRUE} with \code{mean.harvest} and \code{sd.harvest}). In natura, it is
#' unlikely that a population is at maximum annual growth rate (Rmax). Consequently, when harvest exceeds PEG, this suggests an
#' overexploitation regime.
#'
#'
...
#'
#'
#' For more details about the use of the function, see 'README' documentation.
#'
#'
...
#' @param Rmax.fixed number ; fixed estimate of maximum annual growth rate provided by the user. To be used instead of \code{mean.Rmax} when \code{Rmax.lognorm = FALSE}.
#' @param Rmax.lognorm boolean, default \code{FALSE}. If \code{TRUE}, simulates Rmax for a normal distribution with mu = \code{mean.Rmax} and standard deviation = \code{sd.Rmax}.
#' @param mean.Rmax number ; arithmetic mean of maximum annual growth rate.
#' @param sd.Rmax number ; standard deviation of maximum annual growth rate.
#' @param lambdaMax.fixed number ; fixed estimate of maximum multiplicative growth rate provided by the user and used to calculate Rmax as lamndaMax-1.
#' @param lambdaMax.lognorm boolean, default \code{FALSE}. If \code{TRUE}, simulates maximum multiplicative growth rate from a normal distribution with mu = \code{mean.lambdaMax} and standard deviation = \code{sd.lambdaMax}.
#' @param mean.lambdaMax number ; arithmetic mean of maximum multiplicative growth rate. To by used only if \code{lamndaMax.lognorm = TRUE}.
...
#' @param surv.fixed number ; fixed estimate of adult annual survival rate provided by the user. To be used instead of \code{mean.surv} when \code{surv.beta = FALSE}.
#' @param surv.beta boolean, default FALSE. If \code{TRUE}, simulates adult annual survival rate from a beta distribution with mu = \code{mean.surv} and standard deviation = \code{sd.surv} (see below).
#' @param mean.surv number ; arithmetic mean of adult annual survival rate. Only used if \code{surv.beta = TRUE}.
...
#' @param alpha.lognorm boolean, default \code{FALSE}. If \code{TRUE}, simulates age at first breeding from a normal distribution with mu = \code{mean.alpha} and standard deviation = \code{sd.alpha}.
...
#' @param harvest.lognorm boolean, default \code{FALSE}. If \code{TRUE}, simulates harvest level from a normal distribution with mu =  \code{mean.harvest} and standard deviation = \code{sd.harvest}.
...
#' @param pop.lognorm boolean, default \code{FALSE}.  If \code{TRUE}, simulates a population size from a normal distribution with mu = \code{mean.pop} and standard deviation = \code{sd.pop}.
...
PEG <- function(full.option = FALSE,
  ...
  if( isTRUE(alpha.lognorm) && (isTRUE(alpha.unif) || isTRUE(mass.lognorm)) ){
    stop("'alpha.unif' and 'alpha.lognorm' or 'mass.lognorm'  can not be TRUE together")
  }
  ...
  ## III. Calculations ----
  ## Step 1 : Creation of the end storage vectors to save results of the calculations and loading functions----
  Output1 <- NULL
  Output2 <- NULL
  ...
  for (a in 1:NSp){ # Loop for calculations from 1 to the number of species studied
    ...
    surv.fixedT <- ifelse(!is.null(surv.fixed), surv.fixed[a], NA) # If the argument 'surv.fixed' is not null, then temporary setting the value of 'surv.fixed' of the index a in the object named 'surv.fixedT', else atributting 'NA' to the object 'surv.fixedT'. The same philosophy is applied below.
    ...
    # This step makes it possible to repeat the series of calculations whithin the loop as many times as there are safety factors
    for (b in 1:length(Fs)){ # Loop for calculations according the safety factor
      FsT <- Fs[b]
      ...
      # In this case, Rmax values are generated without calculations
        ...
        if(isTRUE(Rmax.lognorm)){ # If Rmax is extrated from a normal distribution then :
          ...
          Rmax.calc <- sample(seq(Rmax.param[1]-Rmax.param[2], Rmax.param[1]+Rmax.param[2], 0.001), Nsim) # 'Nsim' values are extracted from a log-normal distribution with with the mean 'mean.Rmax' and the standard deviation 'sd.Rmax'
        }
        if(isTRUE(lambdaMax.lognorm)){ # If lambdaMax is extracted from a normal distribution then :
          ...
          lambdaMax <- sample(seq(lambdaMax.param[1]-lambdaMax.param[2], lambdaMax.param[1]+lambdaMax.param[2], 0.001), Nsim) # 'Nsim' values are extracted from a log-normal distribution with with the mean 'mean.lambdaMax' and the standard deviation 'sd.lambdaMax'
      ...
      ## Generation of the estimated variables of 'harvest' ----
      ...
      if(isTRUE(harvest.lognorm)){ # If harvest is extracted from a normal distribution then :
        ...
        harvest.calc <- sample(seq(harvest.param[1]-harvest.param[2], harvest.param[1]+harvest.param[2], 0.001), Nsim) # 'Nsim' values are extracted from a log-normal distribution with with the mean 'mean.harvest' and the standard deviation 'sd.harvest'
        harvest.calc <- round(harvest.calc) # Round the values of 'harvest' object to the whole
      }
      ...
      if(isTRUE(pop.lognorm)){  # If popsize is extracted from a normal distribution then :
        ...
        popsize.calc <- sample(seq(pop.param[1]-pop.param[2], pop.param[1]+pop.param[2], 0.001), Nsim) # 'Nsim' values are extracted from a log-normal distribution with with the mean 'mean.pop' and the standard deviation 'sd.pop'
        popsize.calc <- round(popsize.calc) # Round the values of 'popsize' object to the whole
      }
      ...
      ## Saving all datas generated in the step 5 in a data frame Output 1
      if(exists("OutputT1")){ # If the data frame 'OutputT1' exist (means Rmax was calculated from the rmax function) then :
        Output1 <- rbind(Output1, OutputT1) # Combining by rows all temporary data frame 'OutputT1' for each index a of the loop and saving in the data frame 'Output1'
      }
      ...
      OutputT2 <- data.frame(SP = a, Nsim = NsimT, Fs = FsT, popsize = popsize.calc, harvest = harvest.calc) # Creation of the OutputT2
      ...
      var.storage <- c("Rmax.calc", "mean.RmaxT", "sd.RmaxT", "lambdaMax.fixedT", "mean.lambdaMaxT", "sd.lambdaMaxT",
                       ...
                       "min.popT", "max.popT", "mean.popT", "sd.popT") # Saving in an object all sumpplementary possible columns
      ...
      # Step 10 : Saving all the data from the 'OutputT2' in the data frame named 'Output2' ----
    ...
    } # End of loop b (according the safety factor)
  ...
  colnames(full.tab)[colnames(full.tab)=="Nsim"] <- "Sim" # Renames 'Nsim' as 'Sim'
